...
BOX_TEMPLATE_PATH = r'C:\Users\tablrain\Desktop\origin_dev\02_template\BOX.otpu'
...
def calc_grid_positions(layout, slide_width_cm=27.517, slide_height_cm=19.05,
                         left_margin_cm=0.1, right_margin_cm=0.9,
                         gap_cm=0.8, top_margin_cm=1.5,
                         bottom_margin_cm=2.0, row_gap_cm=0.8):
    """
    ...
    """
    max_cols = max(layout)
    usable_width  = slide_width_cm - left_margin_cm - right_margin_cm
    ...
    for row_idx, n_in_row in enumerate(layout):
        ...
        for col_idx in range(n_in_row):
            left_cm = left_margin_cm + col_idx * (cell_width_cm + gap_cm)
            positions.append((
                cm_to_pt(left_cm),
                cm_to_pt(top_cm),
...
def insert_picture_to_slide(prs, pptx_app, target_slide, image_path,
                             left_pt, top_pt, width_pt, height_pt,
                             title, stem, log_fn=print):
    """
    PNG 이미지 파일을 PPT의 지정 슬라이드/위치에 그림으로 삽입
    (Origin OLE 대신 matplotlib PNG를 직접 삽입하는 경로)
    """
    ppt_hwnd = find_ppt_hwnd()
    if ppt_hwnd:
    ...
    pic_shape = current_slide.Shapes.AddPicture(
        FileName=image_path,
        LinkToFile=False,
    ...
    log_fn(f"    └── 슬라이드 {target_slide} '{title}' 이미지 삽입 완료 ✅")
...
def _build_boxplot(origin, ws_name, channel_data, y_col, title, log_fn):
    ...
    Box Chart는 LabTalk Plot Type ID 206(Box, Y Range)을 사용합니다
    (OriginLab 공식 문서 기준: worksheet -p 206;).
    """
    if not channel_data:
    ...
    # ── [0] 채널 수만큼 컬럼 확보 (새 워크시트 기본 컬럼 부족 방지) ──
    current_col_count = wks.Columns.Count
    needed_col_count  = len(ch_nums)
    if current_col_count < needed_col_count:
        origin.Execute(f"wks.ncols = {needed_col_count};")
        time.sleep(0.3)
        wks = origin.FindWorksheet(ws_name)  # 컬럼 추가 후 재조회
        log_fn(f"      └── 컬럼 {current_col_count} → {needed_col_count}개로 확장")
    ...
    for col_idx, ch_num in enumerate(ch_nums):
        ...
        log_fn(f"      ├── col{col_idx+1} = CH{int(ch_num):03d} ({len(values)}개 값)")
        time.sleep(0.03)
    ...
    # ── [2] 워크시트 전체 선택 + Box Chart로 플롯 (ID 206, Y Range) ──
    origin.Execute(f"win -a {ws_name};")
    time.sleep(0.5)
    origin.Execute("worksheet -s all;")
    time.sleep(0.5)
    origin.Execute("worksheet -p 206;")
    time.sleep(3)
...
def _process_rpt_slide(origin, prs, pptx_app, slide_cfg, filtered_df, channels,
    ...
    for idx, cfg in enumerate(graph_cfgs):
        ...
        wks = origin.FindWorksheet(ws_name)
        if wks is None:
            log_fn(f"    ⚠️ FindWorksheet({ws_name}) 1차 실패 → 재시도")
            time.sleep(1.5)
            wks = origin.FindWorksheet(ws_name)
        if wks is None:
            log_fn(f"    ⚠️ FindWorksheet({ws_name}) 최종 실패 → 스킵")
            continue
        ...
        if graph_type == "line":
            ...
            log_fn(f"    ├── 전체 채널: {channels}")
            log_fn(f"    ├── 데이터 있는 채널: {sorted(channel_data.keys())}")
            for ch_check in channels:
                if ch_check not in channel_data:
                    log_fn(f"    ⚠️ CH{int(ch_check):03d}: '{y_col}' 데이터 없음 (전체 NaN이거나 컬럼 누락)")
            if not channel_data:
                log_fn(f"    ⚠️ 데이터 없음 → 스킵")
                continue
