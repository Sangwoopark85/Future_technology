...
# ══════════════════════════════════════════════════════════════
# 경로 설정 (PyInstaller 배포 대응)
# ══════════════════════════════════════════════════════════════

def get_base_dir():
    """
    실행 환경에 따라 기준 경로를 반환.
    - PyInstaller onefile 로 빌드된 경우 : sys._MEIPASS (임시 압축 해제 폴더)
    - PyInstaller onedir 로 빌드된 경우  : exe가 있는 폴더 (sys.executable 위치)
    - 일반 python 스크립트로 실행한 경우 : 이 파일이 있는 폴더
    """
    if getattr(sys, 'frozen', False):
        # onefile 모드는 _MEIPASS를 우선 사용, onedir 모드는 exe 폴더 사용
...
def resource_path(*relative_parts):
    """
    리소스(폰트/로고 등)의 절대경로를 반환.
    빌드 시 'resources' 폴더 아래에 같은 구조로 파일을 넣어두면
    개발 환경(python 실행)과 배포 환경(exe 실행) 모두에서 동일하게 동작.
    """
    return os.path.join(get_base_dir(), "resources", *relative_parts)


def writable_base_dir():
    """
    실행 이력(run_history.json) 등 '쓰기'가 필요한 파일의 기준 경로.
    onedir/onefile 배포본은 설치 폴더에 쓰기 권한이 없을 수도 있으므로
    exe가 있는 폴더(또는 스크립트 폴더)를 그대로 사용.
    PyInstaller onefile의 _MEIPASS는 임시폴더라 쓰기에 적합하지 않음에 주의.
    """
    if getattr(sys, 'frozen', False):
        return os.path.dirname(sys.executable)
    return os.path.dirname(os.path.abspath(__file__))
...
# 배포 시 resources/ 폴더 아래에 다음 파일들을 위치시킬 것:
#   resources/font/LGSmHaR_v1.4_151215.ttf
#   resources/logo/lgensol.png
LG_FONT_PATH = resource_path("font", "LGSmHaR_v1.4_151215.ttf")
LG_LOGO_PATH = resource_path("logo", "lgensol.png")

HISTORY_PATH = os.path.join(writable_base_dir(), "run_history.json")
SETTINGS_ORG = "LGEnS"
SETTINGS_APP = "ChDchTool"

# RPT 슬라이드10 추가 슬롯 개수
RPT_EXTRA_SLOT_COUNT = 4
...
class Worker(QThread):
    ...
    def run(self):
        ...
        try:
            ...
            # PyInstaller로 묶인 경우 같은 폴더(or _MEIPASS)의 모듈을 import 하기 위함
            ...
            if cfg['process'] == 'RPT':
                log(f"  추가 컬럼   : {cfg.get('selected_extra_columns', [])}")
            ...
            run_pipeline(
                ...
                selected_extra_columns = cfg.get('selected_extra_columns', []),
                log_fn     = log,
            )
...
class MainWindow(QMainWindow):
    ...
    def __init__(self):
        ...
        self.setMinimumSize(1050, 980)
        self._settings          = QSettings(SETTINGS_ORG, SETTINGS_APP)
        self._all_channels      = []
        self._selected_channels = []
        self._numeric_columns   = []   # RPT 추가 컬럼 후보 목록
        self._load_lg_font()
        self._build_ui()
        self._apply_style()
    ...
    def _build_ui(self):
        ...
        # [행2.5] RPT 전용 섹션 (조건부 표시)
        top_layout.addWidget(self._build_rpt_extra_group())
        ...
        # 초기 상태: Formation이므로 RPT 섹션 숨김
        self._update_rpt_section_visibility()
    ...
    def _build_process_group(self):
        ...
        self.process_combo.addItem("RPT")
        for p in ["CCA", "Cycle", "Storage", "CP"]:
            self.process_combo.addItem(f"{p}  (준비 중)")
        # RPT 이후(index 2부터)만 비활성화. Formation(0), RPT(1)는 활성화.
        for i in range(2, self.process_combo.count()):
    ...
    # ── RPT 전용: 슬라이드10 추가 컬럼 선택 (조건부 표시) ──────
    def _build_rpt_extra_group(self):
        self.rpt_extra_group = QGroupBox("RPT - 슬라이드 10 추가 Boxplot 컬럼 선택 (최대 4개)")
        layout = QGridLayout(self.rpt_extra_group)
        layout.setContentsMargins(12, 6, 12, 6)
        layout.setSpacing(8)

        self.rpt_extra_status = QLabel(
            "CSV/Parsing 경로를 선택하면 사용 가능한 컬럼이 자동으로 채워집니다."
        )
        self.rpt_extra_status.setObjectName("ch_status")
        self.rpt_extra_status.setWordWrap(True)
        layout.addWidget(self.rpt_extra_status, 0, 0, 1, 4)

        self._rpt_extra_combos = []
        for i in range(RPT_EXTRA_SLOT_COUNT):
            lbl = QLabel(f"슬롯 {i+1} :")
            ...
            self._rpt_extra_combos.append(combo)
            layout.addWidget(combo, 2, i)

        return self.rpt_extra_group
    ...
    def _update_rpt_section_visibility(self):
        is_rpt = (self._get_process() == "RPT")
        self.rpt_extra_group.setVisible(is_rpt)
        if is_rpt:
            self._refresh_numeric_columns()
    ...
    def _on_path_changed(self):
        ...
        if self._get_process() == "RPT":
            self._refresh_numeric_columns()

    def _resolve_active_csv_path(self):
        """현재 모드 기준으로 채널/컬럼 감지에 쓸 CSV 경로를 찾음"""
        mode = 'parsing' if self.chk_parsing.isChecked() else 'csv'
        if mode == 'csv':
            return self._csv_edits['csv_path'].text().strip()
        else:
            ...
            if out_dir and os.path.exists(out_dir):
                ...
                if candidates:
                    # 여러 개 있으면 가장 최근에 수정된 파일을 선택
                    # (이전 데이터셋의 결과물이 같은 폴더에 남아있는 경우 대응)
                    return max(candidates, key=os.path.getmtime)
        return ""

    def _try_detect_channels(self):
        import pandas as pd
        csv_path = self._resolve_active_csv_path()
    ...
    def _refresh_numeric_columns(self):
        """RPT 모드: cyc CSV에서 숫자형 컬럼 목록을 읽어 드롭다운 4개에 채움"""
        import pandas as pd
        csv_path = self._resolve_active_csv_path()

        # 기존 선택값 보존
        prev_selections = [combo.currentText() for combo in self._rpt_extra_combos]

        if not csv_path or not os.path.exists(csv_path):
            self.rpt_extra_status.setText(
                "CSV/Parsing 경로를 선택하면 사용 가능한 컬럼이 자동으로 채워집니다."
            )
            self._numeric_columns = []
            for combo in self._rpt_extra_combos:
                combo.clear()
                combo.addItem("(선택 안 함)")
            return
        ...
        try:
            ...
            df = pd.read_csv(csv_path, encoding='cp949', nrows=500)
        except Exception:
            self.rpt_extra_status.setText("컬럼 감지 실패")
            return

        numeric_cols = df.select_dtypes(include='number').columns.tolist()
        # 이미 슬라이드9/10에 고정으로 들어가는 컬럼은 후보에서 제외
        exclude = {
            'Capacity', 'Voltage', 'Impedance', 'Impedance_0.1s',
            'Impedance_5s', 'Impedance_30s', 'Impedance_60s',
            'chNo', 'ulCurrentCycleNum', 'ulTotalCycleNum',
        }
        numeric_cols = [c for c in numeric_cols if c not in exclude]

        self._numeric_columns = numeric_cols
        self.rpt_extra_status.setText(
            f"감지된 후보 컬럼: {len(numeric_cols)}개 (슬롯별로 boxplot 컬럼을 선택하세요)"
        )

        for combo, prev in zip(self._rpt_extra_combos, prev_selections):
    ...
    def _get_selected_extra_columns(self):
        result = []
        for combo in self._rpt_extra_combos:
            text = combo.currentText()
            if text and text != "(선택 안 함)":
                result.append(text)
        return result
    ...
    def _run(self):
        ...
        if process == "RPT":
            config['selected_extra_columns'] = self._get_selected_extra_columns()
